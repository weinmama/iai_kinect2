...
Kinect2Bridge::Kinect2Bridge(const rclcpp::NodeOptions& options)
		:rclcpp::Node("Kinect2Bridge", options),
		 sizeColor(1920, 1080),
		 sizeIr(512, 424),
...
bool Kinect2Bridge::start()
{
	if (running)
	{
		RCLCPP_ERROR(get_logger(), "kinect2_bridge is already running!");
		return false;
	}
	if (!initialize())
	{
	...
	if (publishTF)
	{
		RCLCPP_INFO(get_logger(), "Starting Publisher for TF");
	...
	controlThread = std::thread(&Kinect2Bridge::control, this);
...
void Kinect2Bridge::stop()
{
	if (!running)
	{
		RCLCPP_ERROR(get_logger(), "kinect2_bridge is not running!");
		return;
	}
	running = false;
	...
	{
		RCLCPP_ERROR(get_logger(), "could not stop device!");
	}

	if (!device->close())
	{
		RCLCPP_ERROR(get_logger(), "could not close device!");
	}
...
bool Kinect2Bridge::initialize()
{
	...
	bool use_png, bilateral_filter = false, edge_aware_filter = false;
...
//#endif
	...
	declare_and_set_parameter("base_name", base_name, std::string(K2_DEFAULT_NS));
	declare_and_set_parameter("sensor", sensor, std::string(""));
	declare_and_set_parameter("fps_limit", fps_limit, -1.0);
	declare_and_set_parameter("calib_path", calib_path, std::string(K2_CALIB_PATH));
	declare_and_set_parameter("use_png", use_png, false);
	declare_and_set_parameter("jpeg_quality", jpeg_quality, 90);
	declare_and_set_parameter("png_level", png_level, 1);
	declare_and_set_parameter("depth_method", depth_method, depthDefault);
	declare_and_set_parameter("depth_device", depth_dev, -1);
	declare_and_set_parameter("reg_method", reg_method, regDefault);
	declare_and_set_parameter("reg_device", reg_dev, -1);
	declare_and_set_parameter("max_depth", maxDepth, 12.0);
	declare_and_set_parameter("min_depth", minDepth, 0.1);
	declare_and_set_parameter("queue_size", queueSize, 2);
	declare_and_set_parameter("bilateral_filter", bilateral_filter, true);
	declare_and_set_parameter("edge_aware_filter", edge_aware_filter, true);
	declare_and_set_parameter("publish_tf", publishTF, true);
	declare_and_set_parameter("base_name_tf", baseNameTF, base_name);
	declare_and_set_parameter("worker_threads", worker_threads, 4);

	worker_threads = std::max(1, worker_threads);
	threads.resize(worker_threads);

	RCLCPP_ERROR_STREAM(get_logger(), "parameter:" << std::endl
												   << "        base_name: " FG_CYAN << base_name << NO_COLOR << std::endl
																					<< "           sensor: " FG_CYAN << (sensor.empty() ? "default" : sensor) << NO_COLOR << std::endl
																													 << "        fps_limit: " FG_CYAN << fps_limit << NO_COLOR << std::endl
																																					  << "       calib_path: " FG_CYAN << calib_path
																																													   << NO_COLOR
																																													   << std::endl
																																													   << "          use_png: "
			FG_CYAN
					<< (use_png ? "true" : "false") << NO_COLOR << std::endl
													 ...
													 << "        png_level: " FG_CYAN << png_level << NO_COLOR << std::endl
																					  << "     depth_method: " FG_CYAN << depth_method << NO_COLOR << std::endl
																													   << "     depth_device: " FG_CYAN << depth_dev << NO_COLOR << std::endl
																																						<< "       reg_method: " FG_CYAN << reg_method
																																														 << NO_COLOR
																																														 << std::endl
																																														 << "       reg_device: "
			FG_CYAN << reg_dev << NO_COLOR << std::endl
					<< "        max_depth: " FG_CYAN << maxDepth << NO_COLOR << std::endl
													 << "        min_depth: " FG_CYAN << minDepth << NO_COLOR << std::endl
																					  << "       queue_size: " FG_CYAN << queueSize << NO_COLOR << std::endl
																													   << " bilateral_filter: " FG_CYAN << (bilateral_filter ? "true" : "false")
																																						<< NO_COLOR << std::endl
																																						<< "edge_aware_filter: " FG_CYAN
					<< (edge_aware_filter ? "true" : "false") << NO_COLOR << std::endl
...
bool Kinect2Bridge::initRegistration(const std::string& method, const int32_t device, const double maxDepth)
{
	...
	{
		RCLCPP_ERROR_STREAM(get_logger(), "Unknown registration method: " << method);
		return false;
	}
...
bool Kinect2Bridge::initPipeline(const std::string& method, const int32_t device)
{
	...
	else if (method == "cuda")
	{
		...
		packetPipeline = new libfreenect2::CudaPacketPipeline(device);
#else
		RCLCPP_ERROR(get_logger(),"Cuda depth processing is not available!");
		return false;
#endif
	}
	else if (method == "opencl")
	{
		...
		packetPipeline = new libfreenect2::OpenCLPacketPipeline(device);
#else
		RCLCPP_ERROR(get_logger(), "OpenCL depth processing is not available!");
		return false;
#endif
	}
	else if (method == "opengl")
	{
		...
		packetPipeline = new libfreenect2::OpenGLPacketPipeline();
#else
		RCLCPP_ERROR(get_logger(),"OpenGL depth processing is not available!");
		return false;
#endif
	}
	...
	{
		RCLCPP_ERROR_STREAM(get_logger(), "Unknown depth processing method: " << method);
		return false;
	}
...
void Kinect2Bridge::initTopics(const int32_t queueSize, const std::string& base_name)
{
	...
	for (size_t i = 0; i < COUNT; ++i)
	{
		imagePubs[i] = this->create_publisher<sensor_msgs::msg::Image>(base_name + topics[i], qoS);
		compressedPubs[i] = this->create_publisher<sensor_msgs::msg::CompressedImage>(base_name + topics[i] + K2_TOPIC_COMPRESSED, qoS);
	}
	infoHDPub = this->create_publisher<sensor_msgs::msg::CameraInfo>(base_name + K2_TOPIC_HD + K2_TOPIC_INFO, qoS);
	infoQHDPub = this->create_publisher<sensor_msgs::msg::CameraInfo>(base_name + K2_TOPIC_QHD + K2_TOPIC_INFO, qoS);
	infoIRPub = this->create_publisher<sensor_msgs::msg::CameraInfo>(base_name + K2_TOPIC_SD + K2_TOPIC_INFO, qoS);
	deviceStatusThread = std::thread(&Kinect2Bridge::maintainDeviceStatus, this);
}
...
bool Kinect2Bridge::initDevice(std::string& sensor)
{
	...
	if (numOfDevs <= 0)
	{
		RCLCPP_ERROR(get_logger(), "no Kinect2 devices found!");
	...
	RCLCPP_INFO(get_logger(), "Kinect2 devices found: ");
	for (int i = 0; i < numOfDevs; ++i)
	{
		const std::string& s = freenect2.getDeviceSerialNumber(i);
		deviceFound = deviceFound || s == sensor;
		RCLCPP_INFO_STREAM(get_logger(), "  " << i << ": " FG_CYAN << s << (s == sensor ? FG_YELLOW " (selected)" : "") << NO_COLOR);
	}

	if (!deviceFound)
	{
		RCLCPP_ERROR_STREAM(get_logger(), "Device with serial '" << sensor << "' not found!");
		delete packetPipeline;
		return false;
	}
	...
	if (device == 0)
	{
		RCLCPP_INFO(get_logger(), "no device connected or failure opening the default one!");
		return false;
	}
	...
	RCLCPP_INFO(get_logger(), "starting kinect2");
	if (!device->start())
	{
		RCLCPP_ERROR(get_logger(), "could not start device!");
		delete listenerIrDepth;
		delete listenerColor;
		return false;
	}

	RCLCPP_INFO_STREAM(get_logger(), "device serial: " FG_CYAN << sensor << NO_COLOR);
	RCLCPP_INFO_STREAM(get_logger(), "device firmware: " FG_CYAN << device->getFirmwareVersion() << NO_COLOR);
	...
	RCLCPP_DEBUG(get_logger(), "default ir camera parameters: ");
	RCLCPP_DEBUG_STREAM(get_logger(),
			"fx: " FG_CYAN << irParams.fx << NO_COLOR ", fy: " FG_CYAN << irParams.fy << NO_COLOR ", cx: " FG_CYAN << irParams.cx << NO_COLOR ", cy: " FG_CYAN << irParams.cy << NO_COLOR);
	RCLCPP_DEBUG_STREAM(get_logger(),
			"k1: " FG_CYAN << irParams.k1 << NO_COLOR ", k2: " FG_CYAN << irParams.k2 << NO_COLOR ", p1: " FG_CYAN << irParams.p1 << NO_COLOR ", p2: " FG_CYAN << irParams.p2 << NO_COLOR ", k3: "
			FG_CYAN << irParams.k3 << NO_COLOR);

	RCLCPP_DEBUG(get_logger(), "default color camera parameters: ");
	RCLCPP_DEBUG_STREAM(get_logger(),
			"fx: " FG_CYAN << colorParams.fx << NO_COLOR ", fy: " FG_CYAN << colorParams.fy << NO_COLOR ", cx: " FG_CYAN << colorParams.cx << NO_COLOR ", cy: " FG_CYAN << colorParams.cy
																																										<< NO_COLOR);
...
void Kinect2Bridge::initCalibration(const std::string& calib_path, const std::string& sensor)
{
	...
	{
		RCLCPP_WARN(get_logger(), "using sensor defaults for color intrinsic parameters.");
	}

	if (calibDirNotFound || !loadCalibrationFile(calibPath + K2_CALIB_IR, cameraMatrixDepth, distortionDepth))
	{
		RCLCPP_WARN(get_logger(), "using sensor defaults for ir intrinsic parameters.");
	}

	if (calibDirNotFound || !loadCalibrationPoseFile(calibPath + K2_CALIB_POSE, rotation, translation))
	{
		RCLCPP_WARN(get_logger(), "using defaults for rotation and translation.");
	}

	if (calibDirNotFound || !loadCalibrationDepthFile(calibPath + K2_CALIB_DEPTH, depthShift))
	{
		RCLCPP_WARN(get_logger(), "using defaults for depth shift.");
		depthShift = 0.0;
	}
	...
	RCLCPP_DEBUG(get_logger(), "camera parameters used:");
	RCLCPP_DEBUG_STREAM(get_logger(), "camera matrix color:" FG_CYAN << std::endl << cameraMatrixColor << NO_COLOR);
	RCLCPP_DEBUG_STREAM(get_logger(), "distortion coefficients color:" FG_CYAN << std::endl << distortionColor << NO_COLOR);
	RCLCPP_DEBUG_STREAM(get_logger(), "camera matrix ir:" FG_CYAN << std::endl << cameraMatrixIr << NO_COLOR);
	RCLCPP_DEBUG_STREAM(get_logger(), "distortion coefficients ir:" FG_CYAN << std::endl << distortionIr << NO_COLOR);
	RCLCPP_DEBUG_STREAM(get_logger(), "camera matrix depth:" FG_CYAN << std::endl << cameraMatrixDepth << NO_COLOR);
	RCLCPP_DEBUG_STREAM(get_logger(), "distortion coefficients depth:" FG_CYAN << std::endl << distortionDepth << NO_COLOR);
	RCLCPP_DEBUG_STREAM(get_logger(), "rotation:" FG_CYAN << std::endl << rotation << NO_COLOR);
	RCLCPP_DEBUG_STREAM(get_logger(), "translation:" FG_CYAN << std::endl << translation << NO_COLOR);
	RCLCPP_DEBUG_STREAM(get_logger(), "depth shift:" FG_CYAN << std::endl << depthShift << NO_COLOR);
}
...
bool Kinect2Bridge::loadCalibrationFile(const std::string& filename, cv::Mat& cameraMatrix, cv::Mat& distortion) const
{
	...
	{
		RCLCPP_ERROR_STREAM(get_logger(), "can't open calibration file: " << filename);
		return false;
	}
	return true;
}
...
bool Kinect2Bridge::loadCalibrationPoseFile(const std::string& filename, cv::Mat& rotation, cv::Mat& translation) const
{
	...
	{
		RCLCPP_ERROR_STREAM(get_logger(), "can't open calibration pose file: " << filename);
		return false;
	}
	return true;
}
...
bool Kinect2Bridge::loadCalibrationDepthFile(const std::string& filename, double& depthShift) const
{
	...
	{
		RCLCPP_ERROR_STREAM(get_logger(), "can't open calibration depth file: " << filename);
		return false;
	}
	return true;
}
...
void Kinect2Bridge::createCameraInfo(const cv::Size& size, const cv::Mat& cameraMatrix, const cv::Mat& distortion, const cv::Mat& rotation, const cv::Mat& projection,
...
void Kinect2Bridge::maintainDeviceStatus()
{
	while (rclcpp::ok())
	{
		...
		lockStatus.lock();
		clientConnected = hasSubscription(isSubscribedColor, isSubscribedDepth);
		bool error = false;

		if (clientConnected && !deviceActive)
		{
			RCLCPP_INFO(get_logger(), "client connected. starting device...");
		...
		{
			RCLCPP_INFO(get_logger(), "no clients connected. stopping device...");
			if (!device->stop())
			{
				RCLCPP_ERROR(get_logger(), "could not stop device!");
				error = true;
...
bool Kinect2Bridge::hasSubscription(bool& isSubscribedColor, bool& isSubscribedDepth)
{
...
void Kinect2Bridge::receiveIrDepth()
{
	...
	if (irFrame->status != 0 || depthFrame->status != 0)
	{
		...
		lockIrDepth.unlock();
		running = false;
		RCLCPP_ERROR(get_logger(), "failure in depth packet processor from libfreenect2");
		return;
	}
	if (irFrame->format != libfreenect2::Frame::Float || depthFrame->format != libfreenect2::Frame::Float)
	{
		...
		lockIrDepth.unlock();
		running = false;
		RCLCPP_ERROR(get_logger(), "received invalid frame format");
		return;
	}
...
void Kinect2Bridge::receiveColor()
{
	...
	if (colorFrame->status != 0)
	{
		...
		lockIrDepth.unlock();
		running = false;
		RCLCPP_ERROR(get_logger(), "failure in rgb packet processor from libfreenect2");
		return;
	}
...
std_msgs::msg::Header Kinect2Bridge::createHeader(rclcpp::Time& last, rclcpp::Time& other)
{
	rclcpp::Time timestamp = rclcpp::Clock(RCL_SYSTEM_TIME).now();
	lockSync.lock();
	if (other.seconds() <= 0.000001)
	{
		last = timestamp;
	}
	else
...
void
Kinect2Bridge::publishImages(const std::vector<cv::Mat>& images, const std_msgs::msg::Header& header, const std::vector<Status>& status, const size_t frame, size_t& pubFrame, const size_t begin,
		const size_t end)
{
...
void Kinect2Bridge::publishStaticTF()
{
	...
	//TODO - treat tf2 stamped correct
	//stColorOpt = tf2::StampedTransform(tZero, now, baseNameTF + K2_TF_LINK, baseNameTF + K2_TF_RGB_OPT_FRAME);
...
void Kinect2Bridge::control()
{
	setThreadName("ControlThread");
	RCLCPP_INFO(get_logger(), "waiting for clients to connect");
		...
		if (now - fpsTime >= 3.0)
		{
			...
			if (isSubscribedDepth)
			{
				RCLCPP_INFO_STREAM(get_logger(),
						"depth processing: " FG_YELLOW "~" << (tDepth / framesIrDepth) * 1000 << "ms" NO_COLOR " (~" << framesIrDepth / tDepth << "Hz) publishing rate: " FG_YELLOW "~"
						<< framesIrDepth / fpsTime << "Hz" NO_COLOR);
			}
			if (isSubscribedColor)
			{
				RCLCPP_INFO_STREAM(get_logger(),
						"color processing: " FG_YELLOW "~" << (tColor / framesColor) * 1000 << "ms" NO_COLOR " (~" << framesColor / tColor << "Hz) publishing rate: " FG_YELLOW "~"
						<< framesColor / fpsTime << "Hz" NO_COLOR);
			}
			fpsTime = now;
		}
