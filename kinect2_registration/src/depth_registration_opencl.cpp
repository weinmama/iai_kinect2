...
#define PRINT_CL_ERROR(expr, err) RCLCPP_ERROR_STREAM(rclcpp::get_logger("DepthRegistrationOpenCl"), FG_BLUE "[" << CL_FILENAME << "]" FG_CYAN "(" << __LINE__ << ") " FG_YELLOW << expr << FG_RED " failed: " << err)
...
bool DepthRegistrationOpenCL::init(const int deviceId)
{
	...
	if (platforms.empty())
	{
		RCLCPP_ERROR(rclcpp::get_logger("DepthRegistrationOpenCl"), "no opencl platforms found.");
		return false;
	}
	...
	RCLCPP_INFO(rclcpp::get_logger("DepthRegistrationOpenCl"),"devices:");
	for (size_t i = 0; i < devices.size(); ++i)
	{
		RCLCPP_INFO_STREAM(rclcpp::get_logger("DepthRegistrationOpenCl"), "  " << i << ": "
		FG_CYAN << deviceString(devices[i]) << NO_COLOR);
	}

	if (!selectDevice(devices, data->device, deviceId))
	{
		RCLCPP_ERROR(rclcpp::get_logger("DepthRegistrationOpenCl"),"could not find any suitable device");
		return false;
	}
	RCLCPP_INFO_STREAM(rclcpp::get_logger("DepthRegistrationOpenCl"), "selected device: "
	FG_YELLOW << deviceString(data->device) << NO_COLOR);
	...
	CHECK_CL_ON_FAIL(data->program.build(options.c_str()),
			RCLCPP_ERROR_STREAM(rclcpp::get_logger("DepthRegistrationOpenCl"), "failed to build program: " << err);
			RCLCPP_ERROR_STREAM(rclcpp::get_logger("DepthRegistrationOpenCl"), "Build Status: " << data->program.getBuildInfo<CL_PROGRAM_BUILD_STATUS>(data->device));
			RCLCPP_ERROR_STREAM(rclcpp::get_logger("DepthRegistrationOpenCl"), "Build Options:\t" << data->program.getBuildInfo<CL_PROGRAM_BUILD_OPTIONS>(data->device));
			RCLCPP_ERROR_STREAM(rclcpp::get_logger("DepthRegistrationOpenCl"), "Build Log:\t " << data->program.getBuildInfo<CL_PROGRAM_BUILD_LOG>(data->device)));
